...
def interpret_pallas_call(
  ...
  def _grid_loop_cond(carry):
    loop_idx, _ = carry
    return loop_idx < num_grid_loop_iterations

  def _grid_loop_body(carry):
    loop_idx, grid_point_coords = carry
    thread_map.thread_map(_kernel, num_threads, grid_point_coords)

    next_loop_idx = loop_idx + 1
    next_grid_point_coords = interpret_utils.get_indices(
        grid_dims, next_loop_idx
    )
    return next_loop_idx, next_grid_point_coords
  ...
  #
  # TODO(nrink): A simple approach to enable some amount of race dection while
  # still processing grid points in a loop (and not concurrently) might be to
  # randomize the order in which grid points are traversed in the loop. This
  # would, however, still lead to deadlock when interpreting a kernel that
  # relies on, e.g., synchronization between grid points, i.e. thread blocks,
  # that are expected to execute concurrently. (Note that the TPU kernel
  # interpreter features randomization along grid dimensions with 'parallel'
  # dimensions semantics.)
  initial_loop_idx = 0
  initial_grid_point = interpret_utils.get_indices(grid_dims, initial_loop_idx)
  jax.lax.while_loop(
      _grid_loop_cond, _grid_loop_body, (initial_loop_idx, initial_grid_point)
  )
